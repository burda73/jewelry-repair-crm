...
export const envSchema = z
  .object({
    ...
    /*
     * Общий предел свежести отчётов, секунды.
     *
     * Применяется как ВЕРХНЯЯ граница к сроку из `REPORT_TTL_MS`: отчёт,
     * устаревающий быстро («просрочки» — 5 минут), настройкой не удлиняется.
     * Годовой срез (сутки) не укорачивается.
     */
    REPORT_CACHE_TTL_SECONDS: z.coerce.number().int().positive().default(900),
    /*
     * Порог, после которого выгрузка считается «тяжёлой» и должна уходить в
     * асинхронную задачу (задача 5.13).
     *
     * НЕ ЧИТАЕТСЯ НАМЕРЕННО: сама асинхронная выгрузка не реализована —
     * материализованные представления и очередь задач отложены. Настройка
     * объявлена заранее, чтобы контракт был виден, но её изменение не имеет
     * эффекта. Это НЕ тот же случай, что `JWT_REFRESH_SECRET` (дефект 51): там
     * настройка считалась работающей защитой, здесь отсутствующая функция
     * описана в `docs/06-reporting.md` §«Асинхронная выгрузка».
     */
