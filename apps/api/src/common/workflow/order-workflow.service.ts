    ...
    for (const guard of guards) {
      switch (guard) {
        ...
        case 'PICKUP_SIGNATURE':
          /*
           * Условие БОЛЬШЕ НЕ ИСПОЛЬЗУЕТСЯ ни одним переходом: заказчик решил,
           * что подпись не должна блокировать выдачу. Ветка оставлена, чтобы
           * правило можно было вернуть одной строкой в таблице переходов, —
           * но намеренно ничего не проверяет.
           *
           * Если сюда снова поставят проверку, выдача опять начнёт требовать
           * росчерк, причём незаметно: интерфейс поле подписи уже не предлагает.
           */
          break;
