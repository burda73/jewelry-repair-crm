...
describe('Хранение и срок жизни (задача 5.7)', () => {
  ...
  /*
   * Настройка `REPORT_CACHE_TTL_SECONDS` была объявлена в схеме окружения, но не
   * читалась нигде: кэш жил строго по таблице `REPORT_TTL_MS`, и изменение
   * переменной не влияло ни на что. Тот же класс дефекта, что «Дефект 41», 50, 51.
   */
  describe('настройка REPORT_CACHE_TTL_SECONDS', () => {
    it('без настройки поведение прежнее', () => {
      expect(ttlWithSetting(REPORT_NAME.REVENUE, 30, undefined)).toBe(
        REPORT_TTL_MS[REPORT_NAME.REVENUE],
      );
    });

    it('укорачивает срок для отчётов с большим TTL', () => {
      // На пилоте руководителю нужны свежие цифры: час на «сроки по этапам»
      // слишком долго, и настройка позволяет это исправить.
      expect(ttlWithSetting(REPORT_NAME.STAGE_DURATIONS, 30, 120)).toBe(120_000);
    });

    it('НЕ удлиняет срок отчёта, у которого он короче', () => {
      /*
       * «Просрочки» живут 5 минут, потому что устаревают быстрее всех.
       * Настройка — общий предел, а не приказ: увеличение переменной до часа
       * не должно делать самый чувствительный отчёт менее свежим, чем задумано
       * доменом.
       */
      expect(ttlWithSetting(REPORT_NAME.OVERDUE, 7, 3600)).toBe(REPORT_TTL_MS[REPORT_NAME.OVERDUE]);
    });

    it('годовой срез настройкой не укорачивается', () => {
      // Годовой отчёт меняется медленно: пересчёт раз в 15 минут был бы тратой
      // без пользы, и настройка по умолчанию (900 с) его не трогает.
      expect(ttlWithSetting(REPORT_NAME.REVENUE, 365, 900)).toBe(24 * 60 * 60_000);
    });

    it('неположительное значение настройки игнорируется', () => {
      // Ноль означал бы «кэш не работает»: отчёт считался бы заново на каждый
      // запрос, и выглядело бы это как замедление без причины.
      expect(ttlWithSetting(REPORT_NAME.REVENUE, 30, 0)).toBe(REPORT_TTL_MS[REPORT_NAME.REVENUE]);
    });
  });
});
