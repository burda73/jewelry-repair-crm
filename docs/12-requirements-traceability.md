...
| 2.6 | Формирование партий по графику | `docs/07` §8.4, `docs/03` §2 | `batches.service.ts` → `create()`, `Counter` `BATCH:ГГГГММДД` | `batches.service.spec.ts` (номер = плановая дата) |
...
| 2.6 | Лимит состава партии | `docs/07` §8.4 | `Setting.logistics.batchMaxItems`, `exceedsBatchLimit()` | `batches.service.spec.ts` (лимит выключен по умолчанию) |
