...
| 2.6 | Формирование партий по графику | `docs/07` §8.5, `docs/03` §2 | `batches.service.ts` → `create()`, `Counter` `BATCH:ГГГГММДД` | `batches.service.spec.ts` (номер = плановая дата) |
...
| 2.6 | Лимит состава партии | `docs/07` §8.5 | `Setting.logistics.batchMaxItems`, `exceedsBatchLimit()` | `batches.service.spec.ts` (лимит выключен по умолчанию) |
...
| 2.6 | Фотофиксация партии | `docs/07` §8.4 | `batches.service.ts` → `uploadPhotos()` | `batches.service.spec.ts` (14 тестов) |
| 2.6 | Загружать фото можно и при приёмке | `docs/07` §8.4 | `canUploadBatchPhoto()` | `batches.spec.ts` (падает при сужении списка) |
| 2.6 | Удалять фото только до отправки | `docs/07` §8.4 | `canDeleteBatchPhoto()` | `batches.spec.ts`, `batches.service.spec.ts` (409) |
| 2.6 | Ключ хранилища не отдаётся наружу | `docs/07` §8.4 | `listPhotos()` → только `url` | `batches.service.spec.ts` (подмена на objectKey роняет тест) |
| 2.6 | Файл-призрак не остаётся на диске | `docs/07` §8.4 | `uploadPhotos()` → `storage.remove` | `batches.service.spec.ts` (снятие уборки роняет тест) |
