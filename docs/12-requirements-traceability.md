...
| Запрос заказчика (дефект 77) | Карточка обновляется после выдачи и приёмки работы | `docs/15` «Дефект 77» | `invalidateOrderCard`; мутации выдачи и приёмки возвращают `OrderAssignment` | Живая проверка: карточка отдаёт `IN_WORK` и ФИО исполнителя сразу после выдачи |
| Запрос заказчика (дефект 78) | «Выдать работу» и «Принять работу» в одном блоке | `docs/08` §3, `docs/15` «Дефект 78» | Единый блок в разделе «Производство»; видимость по `availableTransitions` | Условия взаимоисключающие: `IN_WORK` в «Принят цехом», `WORK_COMPLETED` в «Выдано в работу» |
| Переименование кнопок | «Перевести статус» → «Изменить статус»; «Сбросить пароль» → «Установить пароль» | `docs/08` §1 | `t.order.transition`, `t.users.resetPassword` (+`resetTitle`, `resetDone`) | Тексты приведены к «установить» во всех связанных подписях |
| Запрос заказчика (дефект 79) | Откат заказа до любого состояния истории для администратора | `docs/07` §1.2.3, `docs/02` §4, `docs/15` «Дефект 79» | Право `order:rollback`, `checkOrderRollback`, `rollbackTargets`, `OrderRollbackService`, `RollbackDialog` | 13 тестов домена, 16 тестов API. Живая проверка: список без текущего, 400 без причины, 409 на состояние не из истории, откат выполнен, срок `null`, в ленте «Откат статуса: …». Мутации: терминальность — 1; версия — 1; срок — 1; история — 1 |
| Запрос заказчика (дефект 80) | Цех заказа устанавливается из партии передачи | `docs/15` «Дефект 80» | `moveBatch` в `batches.service.ts`: `workshopId = batch.toWorkshopId` при приёмке рейса «в цех» | 7 тестов API. Живая проверка: заказ без цеха → партия → приёмка → «Центральный цех»; заранее назначенный цех не перезаписан; возврат «в магазин» стал возможен. Мутации: не проставлять — 3; перезаписывать — 1; при отправке — 1 |
| Решение заказчика (дефект 81) | Выдача заказа без подписи клиента | `docs/04` §6, `docs/07` §1, `docs/15` «Дефект 81» | Переходы 18 и 21: `guards: [PAID_IN_FULL]`; `offersPickupSignature`; обезвреженная ветка `PICKUP_SIGNATURE` | 4 теста домена, 13 веб. Живая проверка: выдача без подписи `200` → `COMPLETED`, подпись `null`; до выдачи подпись прикладывается `201`. Мутации: вернуть подпись — 2; убрать оплату — 1; требовать файл — 1; вернуть подсказку — 1 |
| Решение заказчика (дефект 85) | Парольная политика: минимум 6 символов, требований к составу символов нет | `docs/10` §3.1, `docs/15` «Дефект 85» | `passwordSchema`, `isStrongEnoughPassword`, `passwordPolicyViolations`, `passwordHint` в i18n | 4 теста домена (включая «от противного»: каждый случай нарушает ровно одно снятое правило), 3 теста веб. Мутации: вернуть `min(12)` — 2 теста; вернуть требование строчной буквы — 2; требование цифры — 2; вернуть 12 в `isStrongEnoughPassword` — 3; вернуть 12 в интерфейсе — 2. Сгенерированные пароли не ослаблены: `generatePassword` по-прежнему 20 символов. Попутно исправлен `docs/01`: документ обещал проверку по словарю частых паролей, которой в коде никогда не было |
| Находка (тесты) | Фиксированная дата в тесте — отложенное падение | `docs/15` «Дефект 84» | `draft-autosave.spec.ts`: дата относительно `Date.now()` | Тест черновика падал сам по себе через 7 дней после написания (`DRAFT_MAX_AGE_MS`), без единой правки в коде, — выглядело как регресс от текущей задачи. Правило истечения не ослаблено: проверяется отдельным тестом с относительными датами |
