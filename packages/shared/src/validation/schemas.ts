...
/**
 * Парольная политика: минимум 6 символов (решение заказчика).
 *
 * ИЗМЕНЕНИЕ ПОЛИТИКИ. Раньше требовалось 12 символов и наличие строчной,
 * прописной буквы и цифры. Заказчик снял требования к составу символов и снизил
 * длину до 6 — сотрудники приёмки вводят пароль с телефона и планшета, а
 * требования к регистру и цифрам на такой клавиатуре превращаются в источник
 * ошибок входа, которые выглядят как «пароль не подошёл».
 *
 * Запас прочности обеспечивается не составом символов, а хешированием Argon2id
 * (`docs/10-nfr-security.md`): при пароле в 6 символов именно оно, а не политика,
 * делает перебор дорогим. Верхняя граница 128 остаётся: она защищает от
 * переполнения при хешировании, а не ограничивает пользователя.
 *
 * Сообщения об ошибках сохранены по смыслу прежними: причина отказа должна быть
 * конкретной, иначе сотрудник не понимает, что исправить (дефект 45).
 */
 ...
 * Вход в систему.
 *
 * Идентификатор принимается в двух видах, и это НЕ ослабление проверки:
 *  * `email` — обычный вход и все существующие сценарии (скрипты, интеграции,
 *    аварийный доступ, когда список сотрудников выключен или не загрузился);
 *  * `userId` — вход из выпадающего списка сотрудников. Список отдаёт имена и
 *    идентификаторы, но НЕ почтовые адреса: почта — это и персональные данные, и
 *    готовый список адресов для фишинга, а показывать её незачем.
 *
 * Ровно один из двух обязан присутствовать. `refine`, а не `or`: при `or`
 * сообщение об ошибке относилось бы к «ни одному из вариантов» и пользователь не
 * понял бы, чего не хватает.
 */
...
export const organizationRequisitesSchema = z.object({
  ...
  /*
   * Необязательные поля принимают и строку, и `null`, и отсутствие поля.
   *
   * `null` здесь обязателен, а не «на всякий случай»: именно в `null` пустое
   * значение приходит из GET и сохраняется в базе («не заполнено» имеет одно
   * представление). Без `null` клиент, прочитавший реквизиты и отправивший их
   * обратно без изменений, получал бы 400 на пустом ИНН — то есть сохранение
   * настроек ломалось бы ровно тогда, когда ничего не меняли. Проверено на
   * боевом сервере: PUT с `inn: null` отвечал `VALIDATION_ERROR`.
   */
...
export const createBatchSchema = z
   ...
   * Партия «в цех» отправляется из магазина, партия «в магазин» — из цеха.
   *
   * Проверка стоит здесь, а не только в сервисе: `fromStoreId` у партии «в
   * магазин» прежде приходил из формы, и именно поэтому в акте отправителем
   * оказывался магазин. Запрос, который всё ещё пытается его прислать, должен
   * получить понятный отказ, а не молча проигнорированное поле.
   */
  .refine((data) => data.direction !== 'TO_STORE' || data.fromStoreId === undefined, {
    message: 'Партию в магазин отправляет цех — магазин-отправитель не указывается',
    path: ['fromStoreId'],
  });
...
/**
 * Телефон при правке учётной записи.
 *
 * Пустая строка — это осознанное «удалить телефон», а не ошибка ввода: без неё
 * очистить уже сохранённый номер было невозможно, и администратор, стирая
 * значение в карточке, получал `VALIDATION_ERROR` вместо сохранения. `null`
 * принимается по той же причине: так поле приходит от клиентов, которые
 * передают отсутствующее значение явно.
 *
 * Значение обрезается ДО проверки: строка из одних пробелов — тоже «пусто», и
 * отклонять её как «слишком короткий телефон» значило бы запрещать очистку поля,
 * в котором остался случайный пробел.
 */
