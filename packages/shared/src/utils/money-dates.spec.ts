...
/** Календарь без праздников: только выходные (сб, вс). */
...
describe('Даты: рабочие дни (ТЗ п. 2.7, 2.9)', () => {
  it('прибавляет рабочие дни, пропуская выходные', () => {
    // Пятница 2025-01-03 + 1 рабочий день = понедельник 2025-01-06
    const friday = new Date('2025-01-03T10:00:00Z');
    expect(toDateKey(addWorkingDays(friday, 1, plainCalendar))).toBe('2025-01-06');
  });

  it('учитывает праздники из производственного календаря', () => {
    // Вторник 2024-12-31 + 1 рабочий день: 1 января — праздник, значит 2 января
    const tuesday = new Date('2024-12-31T10:00:00Z');
    expect(toDateKey(addWorkingDays(tuesday, 1, holidayCalendar))).toBe('2025-01-02');
  });
