...
export const NOTIFICATION_TEMPLATES: readonly NotificationTemplateSeed[] = [
  ...
  /*
   * Предупреждение о сроке рекламации (задача 6.6). Почтовый текст отличается от
   * текста в интерфейсе: в письме нельзя «нажать на рекламацию», поэтому
   * указывается, что именно нужно сделать до срока. Соответствие «код + канал»
   * обязательно — без пары `EMAIL` сервис отправил бы запасной текст вида
   * «Событие: CLAIM_DEADLINE» (реальный дефект задачи 5.9).
   */
  {
    channel: TEMPLATE_CHANNEL.EMAIL,
    code: 'CLAIM_DEADLINE',
    subject: 'Приближается срок рекламации {{claimNo}}',
    body:
      'Срок рассмотрения рекламации {{claimNo}} истекает {{dueDate}}.\n\n' +
      'Срок установлен ТЗ: 10 рабочих дней на рассмотрение обращения. Если решение ' +
      'ещё не принято, разберите рекламацию до этой даты — иначе срок будет нарушен.',
  },
