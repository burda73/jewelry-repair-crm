...
describe('Реестр отчётов', () => {
  it('содержит пять отчётов ТЗ п. 2.11 и отчёт по рекламациям этапа 6', () => {
    expect(ALL_REPORT_NAMES).toHaveLength(6);
    // Имена совпадают с контрактом docs/07 §12.
    // `claims` — задача 6.7: рекламации выделены в отдельный отчёт, потому что
    // у них собственный срок (10 рабочих дней) и собственные исходы.
