...
/**
 * Кто ОТПРАВИТЕЛЬ партии — по направлению, а не по выбору сотрудника.
 *
 * ## Дефект, который здесь закрывается
 *
 * При формировании отправки из цеха в магазин отправителем указывался МАГАЗИН.
 * Причина структурная: у партии было только поле `fromStoreId`, поля «цех-
 * отправитель» не существовало вовсе, и форма предлагала выбрать магазин
 * независимо от направления. В акте приёма-передачи, который подписывают обе
 * стороны, отправителем значился магазин — то есть документ называл не того,
 * кто фактически передал изделия.
 *
 * ## Почему отправитель выводится, а не выбирается
 *
 * Отправитель — это СЛЕДСТВИЕ маршрута, а не решение сотрудника:
 *
 *  * партия «в цех» собирается в магазине — отправляет магазин;
 *  * партия «в магазин» собирается в цехе — отправляет цех.
 *
 * Пока это был свободный выбор, ошибка была вопросом времени, и она случилась.
 * Убрать выбор совсем — единственный способ не повторять её при каждой отправке.
 *
 * ## Почему цех берётся из заказов партии
 *
 * Привязки «пользователь → цех» в системе нет (`user_store` есть, `user_workshop`
 * нет), поэтому цех отправки определяется по САМИМ ЗАКАЗАМ партии: изделия
 * физически находятся там, где их ремонтировали. Это надёжнее привязки
 * сотрудника: даже если менеджер переведён в другой цех, документ назовёт цех,
 * откуда изделия действительно уехали.
 */
export const BATCH_SENDER = {
  STORE: 'STORE',
  WORKSHOP: 'WORKSHOP',
} as const;

export type BatchSenderKind = (typeof BATCH_SENDER)[keyof typeof BATCH_SENDER];

/**
 * Кто отправляет партию такого направления.
 *
 * `TO_PRODUCTION` — магазин отправляет изделия в цех.
 * `TO_STORE` — цех возвращает готовые изделия в магазин.
 */
export function batchSenderKind(direction: BatchDirection): BatchSenderKind {
  return direction === BATCH_DIRECTION.TO_PRODUCTION ? BATCH_SENDER.STORE : BATCH_SENDER.WORKSHOP;
}

/**
 * Цех-отправитель партии «в магазин» по её заказам.
 *
 * Возвращается `null`, если определить не удалось (в партии ещё нет заказов или
 * у них не заполнен цех). Вызывающий обязан отказать в создании партии с
 * понятным объяснением: подставить сюда произвольный цех значило бы снова
 * напечатать в акте не того отправителя — ровно то, на что жаловался заказчик.
 *
 * Если заказы ремонтировались в РАЗНЫХ цехах, отправка одной партией невозможна
 * физически: изделия лежат в разных местах, и один акт на них подписать нельзя.
 */
export function resolveBatchSourceWorkshop(
  workshopIds: readonly (string | null | undefined)[],
):
  | { ok: true; workshopId: string }
  | { ok: false; reason: 'NO_ORDERS' | 'WORKSHOP_UNKNOWN' | 'MIXED' } {
  if (workshopIds.length === 0) return { ok: false, reason: 'NO_ORDERS' };

  const known = workshopIds.filter(
    (id): id is string => id !== null && id !== undefined && id !== '',
  );
  if (known.length === 0) return { ok: false, reason: 'WORKSHOP_UNKNOWN' };

  const unique = [...new Set(known)];
  if (unique.length > 1) return { ok: false, reason: 'MIXED' };

  // Часть заказов без цеха — отправлять нельзя: неизвестно, где часть изделий.
  if (known.length !== workshopIds.length) return { ok: false, reason: 'WORKSHOP_UNKNOWN' };

  return { ok: true, workshopId: unique[0]! };
}

/** Объяснение сотруднику, почему партию нельзя создать. */
export function batchSourceWorkshopMessage(
  reason: 'NO_ORDERS' | 'WORKSHOP_UNKNOWN' | 'MIXED',
): string {
  switch (reason) {
    case 'NO_ORDERS':
      return 'Добавьте в партию хотя бы один заказ — по ним определяется цех-отправитель';
    case 'MIXED':
      return 'Заказы партии ремонтировались в разных цехах — отправка одной партией невозможна';
    case 'WORKSHOP_UNKNOWN':
    default:
      return 'У заказов партии не указан цех — отправка невозможна: в акте нечего указать как отправителя';
  }
}
