...
export const PERMISSION = {
  ...
  /**
   * Откат заказа до любого состояния из его истории.
   *
   * Право есть ТОЛЬКО у администратора, и это не формальность: откат обходит
   * таблицу переходов, то есть отменяет её гарантии. Таблица описывает, какие
   * переходы допустимы при каких условиях, и именно она защищает от «выдали
   * изделие, а потом вернули в работу». Такой обход допустим как аварийный
   * инструмент разбора ошибок, а не как рабочий приём, поэтому он изъят у всех
   * ролей, включая руководителя: исправление ошибок в статусах — задача
   * администратора, а не ежедневная операция.
   */
