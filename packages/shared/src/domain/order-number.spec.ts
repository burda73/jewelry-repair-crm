...
/**
 * Дата в номере документа — рабочая дата точки (Москва), а не UTC.
 *
 * Все проверки построены на моментах 21:00–00:00 UTC: именно там UTC и Москва
 * расходятся по КАЛЕНДАРНОЙ дате. Ошибка не косметическая — по номеру ищут
 * документ, а отчётность группирует заказы по месяцу из номера.
 */
describe('Номер документа: дата по московскому времени', () => {
  it('заказ, принятый ночью 1 октября, получает октябрьский номер', () => {
    // 30.09.2025 22:00 UTC = 01.10.2025 01:00 МСК. С прежним `getUTCMonth()`
    // номер был бы за сентябрь.
    expect(buildOrderNo('MSK1', new Date('2025-09-30T22:00:00Z'), 4)).toBe('MSK1-2510-000004');
  });

  it('номер партии содержит московскую дату, а не UTC', () => {
    // 16.09.2025 22:00 UTC = 17.09.2025 01:00 МСК.
    expect(buildBatchNo(new Date('2025-09-16T22:00:00Z'), 4)).toBe('П-250917-004');
  });

  it('заказ, принятый в новогоднюю ночь, попадает в январский счётчик', () => {
    // 31.12.2025 22:00 UTC = 01.01.2026 01:00 МСК.
    expect(orderCounterScope('MSK1', new Date('2025-12-31T22:00:00Z'))).toBe('ORDER:MSK1:202601');
  });

  it('год в номере акта берётся по Москве', () => {
    // 31.12.2025 22:00 UTC = 01.01.2026 01:00 МСК: с UTC год был бы 25.
    expect(buildBatchActNo(new Date('2025-12-31T22:00:00Z'), 1)).toBe('АПП-26-000001');
    expect(buildRefusalActNo(new Date('2025-12-31T22:00:00Z'), 7)).toBe('АО-26-000007');
    expect(buildClaimNo(new Date('2025-12-31T22:00:00Z'), 31)).toBe('РЕК-26-00031');
  });

  it('днём московская и UTC-дата совпадают', () => {
    // Проверка, что переход на Москву не сдвинул обычные дневные случаи.
    const noon = new Date('2025-09-16T10:00:00Z');
    expect(buildBatchNo(noon, 4)).toBe('П-250916-004');
    expect(buildOrderNo('MSK1', noon, 4)).toBe('MSK1-2509-000004');
  });

  it('вечером того же дня номер не меняется', () => {
    // 16.09.2025 20:59 UTC = 23:59 МСК — ещё 16-е.
    expect(buildBatchNo(new Date('2025-09-16T20:59:00Z'), 5)).toBe('П-250916-005');
  });
});
