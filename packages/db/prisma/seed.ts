...
/**
 * Защита от запуска на боевой базе.
 *
 * `seed.ts` создаёт девять демонстрационных учётных записей с ОДНИМ общим
 * паролем, и этот пароль вместе с адресами лежит в открытом репозитории.
 * Случайный `npm run db:seed` на боевой базе создал бы работающие учётные
 * записи с общеизвестным паролем — то есть открыл бы вход в систему.
 *
 * Поэтому в продакшне наполнение запрещено по умолчанию. Обойти запрет можно
 * только явным флагом `SEED_ALLOW_PRODUCTION=yes`: это осознанное действие,
 * а не случайное совпадение обстоятельств.
 */
function assertNotProduction(): void {
  if (process.env.NODE_ENV !== 'production') return;
  if (process.env.SEED_ALLOW_PRODUCTION === 'yes') {
    console.warn(
      'ВНИМАНИЕ: наполнение запущено в продакшне (SEED_ALLOW_PRODUCTION=yes).\n' +
        'Будут созданы демонстрационные учётные записи с общеизвестным паролем.',
    );
    return;
  }

  throw new Error(
    'Наполнение базы запрещено в продакшне: NODE_ENV=production.\n' +
      '  seed.ts создаёт девять демонстрационных учётных записей с общим\n' +
      '  паролем, опубликованным в репозитории.\n' +
      '  Если наполнение действительно нужно, задайте SEED_ALLOW_PRODUCTION=yes.',
  );
}
